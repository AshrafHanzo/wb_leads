...
app.get('/api/leads/users', async (req, res) => {
    try {
        const result = await pool.query("SELECT user_id, full_name as user_name FROM users WHERE status = 'Active' ORDER BY full_name");
        res.json(result.rows);
    } catch (err) {
...
app.listen(port, () => {
    ...
    console.log(`API endpoints available at http://localhost:${port}/api/leads`);
});
